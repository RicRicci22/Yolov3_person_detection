...
model.load_weights(r'C:\Users\Melgani\Desktop\master_degree\trained_weights\custom\custom800.pth')
model.activate_gpu()
...
for resolution in resolutions:
    ...
    ax.plot(range(len(iou_list)),average_prec_list,label='Finetuned model')
    ax2.plot(range(len(iou_list)),average_rec_list,label='Finetuned model')
    # Small
    
    ax6.plot(range(len(iou_list)),small_mean_rec_list,label='Finetuned model')
    # Medium 
    
    ax8.plot(range(len(iou_list)),medium_mean_rec_list,label='Finetuned model')
    # Large
    
    ax10.plot(range(len(iou_list)),large_mean_rec_list,label='Finetuned model')
